...
import com.brianwilens.andro.exception.GlobalExceptionHandler
...
internal class AndroControllerTest {
       ...
       private val testUserBad = User(0,
        ...
        fun givenNewUser_whenNewEmail_thenReturnCreatedResponseEntity() {
            ...
            val expectedResponse: String = ObjectMapper().writeValueAsString(
                    AndroResponse(HttpStatus.CREATED.value(), testUserGood))
        ...
        fun givenNewUser_whenExistingEmail_thenReturnException() {
            //Declaring expected response and necessary variables used uniquely for this test
            val exception = AndroException(ExceptionResponses.EMAIL_EXISTS.status,
                    ExceptionResponses.EMAIL_EXISTS.message)
            val expectedResponse: String = ObjectMapper().writeValueAsString(
                    AndroResponse(exception.status.value(),exception.message))
    ...
    inner class GetUserAndroEndpointTests(){
        ...
        @Test
        fun givenUserId_whenUserExists_thenReturnUserResponseEntity(){
            //Declaring expected response and necessary variables used uniquely for this test
            val expectedResponse: String = ObjectMapper().writeValueAsString(
                AndroResponse(HttpStatus.OK.value(), testUserGood))

            //Mock the usage of our service to separate it from this test
            //In this case the service taps the database and simply returns the found user
            whenever(androService.getUser(testUserGood.userId)).thenReturn(testUserGood)

            //Perform test
            val test: MvcResult = mockMvc.perform(get("/andro/"+testUserGood.userId)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk)
                .andReturn()

            //verify usage of mocked service
            verify(androService, times(1)).getUser(testUserGood.userId)
            verifyNoMoreInteractions(androService)
        ...
        @Test
        fun givenUserId_whenUserNotExists_thenReturnException(){
            //Declaring expected response and necessary variables used uniquely for this test
            val exception = AndroException(ExceptionResponses.USER_NOT_FOUND.status,
            ...
            //Mock the usage of our service to separate it from this test
            //In this case the service taps the database and throws an exception since the user does not exist
            whenever(androService.getUser(testUserBad.userId)).thenThrow(exception)

            //Perform test
            val test: MvcResult = mockMvc.perform(get("/andro/"+testUserBad.userId)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isNotFound)
                .andReturn()

            //verify usage of mocked service
            verify(androService, times(1)).getUser(testUserBad.userId)
            verifyNoMoreInteractions(androService)
    ...
    @Nested
    @DisplayName("Put andro/{userId} Endpoint Tests")
    inner class PutUserAndroEndpointTests(){
